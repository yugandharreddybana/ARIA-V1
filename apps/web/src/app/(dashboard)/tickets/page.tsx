        ...
        <CardContent>
          <form onSubmit={submit} className="space-y-4">
            ...
            <div className="space-y-1.5">
              <Label>Title</Label>
              <Input value={title} onChange={e => setTitle(e.target.value)} placeholder="Short description" autoFocus />
            </div>
            <div className="space-y-1.5">
...
function TicketCard({ ticket, onStatusChange }: { ticket: Ticket; onStatusChange: (id: string, status: TicketStatus) => void }) {
  const nextStatus: Partial<Record<TicketStatus, TicketStatus>> = {
    ...
    in_progress: 'ready_for_qa', ready_for_qa: 'in_qa',
    in_qa: 'done',
  };
  const next = nextStatus[ticket.status];
  return (
    <div className="rounded-lg border border-border bg-card p-3 space-y-2 hover:border-aria-500/40 transition-colors">
...
export default function TicketsPage() {
  ...
  const loadTickets = useCallback(async (pid: string) => {
    ...
    try {
      const d = await api<{ tickets: Ticket[] }>(`/tickets?projectId=${pid}`);
  ...
  const handleStatusChange = async (id: string, status: TicketStatus) => {
    try {
      const d = await api<{ ticket: Ticket }>(`/tickets/${id}`, { method: 'PATCH', body: JSON.stringify({ status }) });
      ...
      {loading ? (
        ...
        <div className="flex gap-4 overflow-x-auto pb-4 flex-1">
          {COLUMNS.map(col => {
            ...
            return (
              <div key={col.key} className={`flex flex-col gap-3 min-w-[240px] max-w-[280px] border-t-2 pt-3 ${col.color}`}>
                <div className="flex items-center justify-between">
